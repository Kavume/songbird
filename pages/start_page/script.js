...
const dropdown = document.querySelectorAll('.dropdown__content');

function renderDropdownString() {

    for (let i = 0; i < birdsData.length; i++) {

        for (let j = 0; j < birdsData[i].length; j++) {

            const dropdownString = document.createElement('p');
            dropdownString.innerHTML = birdsData[i][j].name;
            dropdownString.classList.add('dropdown__string');

            dropdown[i].append(dropdownString);
        }
...
let renderedNumber = birdsData[Math.floor(Math.random() * birdsData.length)][Math.floor(Math.random() * birdsData.length)].audio;

const audio = new Audio();
audio.src = renderedNumber;
...
function init() {
    renderDropdownString();
}

init();

let englishCollator = new Intl.Collator('en');
let polishCollator = new Intl.Collator('pl');

let englishWords = ['apple', 'banana', 'cherry'];
let polishWords = ['jabłko', 'banan', 'wiśnia'];

englishWords.sort(englishCollator.compare);
polishWords.sort(polishCollator.compare);
